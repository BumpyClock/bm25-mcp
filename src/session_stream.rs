...
fn digest_file_controlled(
    ...
    let mut checkpoint_prefix_hasher = Sha256::new();
    let mut previous_prefix_hasher = Sha256::new();
    let mut buffer = [0_u8; 16 * 1024];
    ...
    loop {
        ...
        if offset < checkpoint_offset {
            let end = checkpoint_offset.saturating_sub(offset).min(count as u64) as usize;
            checkpoint_prefix_hasher.update(&bytes[..end]);
        }
        if offset < suffix_offset {
            let end = suffix_offset.saturating_sub(offset).min(count as u64) as usize;
            previous_prefix_hasher.update(&bytes[..end]);
        }
    ...
    Ok((
        ...
        hex_digest(checkpoint_prefix_hasher.finalize()),
        hex_digest(previous_prefix_hasher.finalize()),
    ))
}
